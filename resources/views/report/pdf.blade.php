...
<div id="lower" style ="font-size: 14px;  font-family:Roboto;">
    ...
    <div class="details" style="margin-top: 30px;">
       {{$currentDate}} EST
        ...
        <br>
        <h6 class="mt-4" style="font:Arial; font-size: 15px;"><b>{{$author->getFullName()}}</b></h6>
    </div>
    <div class="details" style="margin-top: 30px; font-family: Roboto;" >
      Dear {{$author->firstname}},<br>
      @if(!empty($totalPods['royalty']))
      Enclosed is the royalty payment amounting to <span>${{substr($totalPods['royalty'],0,-1)}}</span> ({{App\Helpers\NumberFormatterHelper::numtowords(substr($totalPods['royalty'],0,-1)) }}).
      @elseif(!empty($totalEbooks['royalty']))
      Enclosed is the royalty payment amounting to <strong>${{substr($totalEbooks['royalty'],0,-1)}}<!--{{$totalRoyalties}}--></strong> ({{App\Helpers\NumberFormatterHelper::numtowords(substr($totalEbooks['royalty'],0,-1))}}).
      @endif
       ...
       <br>
        ...
        <table style="width:100%;font-size: 14px; font-family:Roboto;">
            <thead style=" background-color:#336EFF ;border: 1px solid;font-size: 12px; color:#EBD5D1;">
                <tr style="font-family:Roboto; text-align:center; ">
            ...
            <tbody style="" >
                @foreach ($pods as $pod)
                    @if(App\Helpers\UtilityHelper::hasTotalString($pod))
                        <tr style="font-family:Roboto;">
                            <td colspan="4"style="font-family:Roboto; border:1px solid; width:90px; background-color:#84A2FF ;"><b style="font-family:">{{$pod['title']}}</b></td>
                ...
                <td colspan="4" style="border: 1px solid; width:90px; font-family:Roboto;  background-color:#336EFF;color: #FFFFFF;"><b>{{$totalPods['title']}}</b></td>
                    <td style="text-align:center "><b>{{$totalPods['quantity']}}</b></td>
                    <td style=" text-align:center"><b>${{$totalPods['price']}}</b></td> 
                    <td style=" text-align:center"><b>${{$totalPods['revenue']}}</b></td>
                    <td style=" text-align:center"><b>${{substr($totalPods['royalty'],0,-1)}}</b></td>
                </tr>
            </tbody>
        </table>
        @endif
        ...
        <thead style=" background-color:#336EFF ;border: 1px solid;font-size: 12px; color:#EBD5D1;">
            ...
            <tbody style="">
                @foreach ($ebooks as $ebook)
                    ...
                    <tr>
                        <td colspan="4"style="font-family:Roboto; border:1px solid; width:90px; background-color:#84A2FF ;"><b>{{$ebook['title']}}</b></td>
                        <td style="border: 1px solid; width:70px; background-color:#84A8FF ; text-align:center;"><b></b></td>
                        <td style="border: 1px solid; width:70px; background-color:#84A8FF ; text-align:center;"><b>{{$ebook['quantity']}}</b></td>
                        <td style="border: 1px solid; width:70px; background-color:#84A8FF ; text-align:center;"><b>{{$ebook['price']}}</b></td>
                        <td style="border: 1px solid; width:70px; background-color:#84A8FF ; text-align:center;"><b>{{$ebook['revenue']}}</b></td>
                        <td style="border: 1px solid; width:70px; background-color:#84A8FF ; text-align:center;"><b>{{$ebook['royalty']}}</b></td>
                    </tr>
                    @else
                    <tr>
    ...
    <h5 style="font-size: 15px; font-family: Roboto; "><span><b style="font-size: 15px;">ReadersMagnet</b></span></h5>
</div>
